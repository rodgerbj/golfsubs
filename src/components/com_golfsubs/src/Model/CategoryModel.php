...
/**
 * Single item model for a member
 *
 * @package     Joomla.Site
 * @subpackage  com_golfsubs
...
class CategoryModel extends ListModel
{
	...
	/**
	 * Array of members in the category
	 *
	 * @var    \stdClass[]
	 */
	...
	/**
	 * The list of other member categories.
	 *
	 * @var    array
	 */
	...
	public function getItems()
	{
		...
		for ($i = 0, $n = count($items); $i < $n; $i++) {
			...
			if ($this->getState('load_tags', true)) {
				...
				$this->tags->getItemTags('com_golfsubs.member', $item->id);
			}
		}
		return $items;
	}
	...
	protected function getListQuery()
	{
		...
		if (!empty($search)) {
			$search = '%' . trim($search) . '%';
			$query->where($db->quoteName('a.memname') . ' LIKE :memname ');
			$query->bind(':memname', $search);
		}
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$params = ComponentHelper::getParams('com_golfsubs');
		...
		$numberOfMembersToDisplay = $mergedParams->get('members_display_num');
		if ($format === 'feed') {
			$limit = $app->get('feed_limit');
		} else if (isset($numberOfMembersToDisplay)) {
			$limit = $numberOfMembersToDisplay;
		} else {
		...
		$search = $app->getUserStateFromRequest('com_golfsubs.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
		...
		$user = Factory::getUser();
		if ((!$user->authorise('core.edit.state', 'com_golfsubs')) && (!$user->authorise('core.edit', 'com_golfsubs'))) {
	...
	public function getCategory()
	{
		if (!is_object($this->_item)) {
			...
			$categories = Categories::getInstance('Golfsubs', $options);
